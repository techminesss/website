...
const CourseModal = ({ isOpen, onClose, course }) => {
  ...
  const [cachedCourse, setCachedCourse] = useState(course);

  useEffect(() => {
    if (course) {
      setTimeout(() => setCachedCourse(course), 0);
    }
  }, [course]);

  const displayCourse = course || cachedCourse;
  ...
  return (
            ...
            >
              ...
              <div className="relative h-56 shrink-0">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  ...
                  <h2 className="text-3xl md:text-4xl font-bold text-white leading-tight shadow-black drop-shadow-lg">{displayCourse.title}</h2>
                </div>
              </div>
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar flex-grow space-y-8">
                ...
                <div>
                   <p className="text-zinc-300 text-sm leading-relaxed">
                     {displayCourse.longDesc || displayCourse.shortDesc}
                   </p>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4 relative pl-2">
                    ...
                    {displayCourse.syllabus && displayCourse.syllabus.map((item, idx) => (
                  ...
                  {displayCourse.testimonial && (
                    <div className="bg-gradient-to-br from-blue-900/20 to-purple-900/20 p-5 rounded-2xl border border-white/5 relative overflow-hidden">
                       ...
                       <div className="relative z-10">
                           ...
                           <p className="text-sm text-zinc-200 italic mb-3">"{displayCourse.testimonial.text}"</p>
                           <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-full bg-zinc-700 flex items-center justify-center text-[10px] font-bold text-white">
                                  {displayCourse.testimonial.author[0]}
                              </div>
                              <p className="text-xs font-bold text-zinc-400">
                                  {displayCourse.testimonial.author} <span className="font-normal opacity-50">• {displayCourse.testimonial.role}</span>
                              </p>
                           </div>
                       </div>
                    </div>
    ...
    <DemoBookingModal 
      ...
      defaultCourse={displayCourse?.title} 
      title={`Book a Free Demo — ${displayCourse?.title}`}
    />
    </>
  );
};
